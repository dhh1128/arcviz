...
**Done, 2026-09-15, amended 2026-09-16.** Arm B is rebuilt with no authoring width — [docs/design/layout-comparison/arm-b-responsive.html](docs/design/layout-comparison/arm-b-responsive.html), results in [RESPONSIVE.md](docs/design/layout-comparison/RESPONSIVE.md). It is the repository's first artifact that passes at 320, and at every width from 300 to 1,920, apparatus included. **Open it before reading anything else about it** — it now opens with a plain-language orientation and carries a reader's key, because the first version assumed the whole repository's vocabulary and was unreadable to its actual reader. Read that file before the retest, which it partly supersedes. The short version: the packing rules are now rules (`n = ⌊(W−24)/84⌋` chips across, `62/n` px of column per object), the retest over-predicted the focus register's growth at 320 by a factor of three, and its 399–453 px range for the rest register turned out to be an unmade decision about packing rather than measurement uncertainty. Four things the fixed-width mocks could not show came out of it, listed under "What the rebuild found" below.
...
**A responsive layout can break a ratified decision by having no ceiling, not only by having no floor.** The first build had no maximum width, so above 1,280 px all thirteen objects packed into one row and the diamond, the shared ancestor and both descending edges disappeared — the layout converging on the horizontal axis DD-7 refused, at every ordinary desktop width. It caps at 480 px now and turns surplus width into margin. **The cap is a design choice nobody has ruled on**; `?maxw=none` renders the degeneration, `?maxw=360` pins the packing every earlier artifact was authored at. The lesson generalises past this file: a width sweep that stops at 800 px is not a width sweep, and the same sweep should cover the exhibit apparatus and not only the component — the readout added to fix this broke the page's own 320 px conformance on the first attempt, which is the retest's required change 6 reproduced in the file written to fix it.
