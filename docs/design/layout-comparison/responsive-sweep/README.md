...
Renders of [../arm-b-responsive.html](../arm-b-responsive.html), clipped to the component itself — no page chrome, no gauge rail, no phone frame. The number in each filename is the viewport width in CSS pixels; each file is that width and no other, because the component has no authoring width. Captured at device pixel ratio 2 in headless Chrome 153. Measurements are in [../RESPONSIVE.md](../RESPONSIVE.md).
...
| `rest-480.png` | Five across. |
| `rest-768.png` | Eight across, two rows. |
