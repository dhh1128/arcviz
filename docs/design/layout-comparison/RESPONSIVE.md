...
**The convention, and a wrong reading of it corrected.** `text-scale.ts` makes text the *inherited host running size* with named steps at 1, 0.85 and 0.72 em, and forbids "ad-hoc per-element magic numbers" — which is exactly what arcviz's 8 / 6.5 / 6 px were. An earlier draft of this section read that as "the pill inherits the *document's* body size, so the type floor is not arcviz's to choose." That is wrong (Daniel Hardman, 2026-09-17): **arcviz is the host.** It can set one font-size on the container it embeds the pill in, and everything inherits. The convention is a rule against scattering magic numbers, not a rule about what the host's size must be, and arcviz now satisfies it — one base, everything derived from it.

Measured against the mounted component, the pill follows that base exactly: label, copy kebab and role icon all render at the host size at every base tested from 8 px to 20 px, and the pill is **≈ 7.5 em wide regardless**. (Checked rather than assumed — the source carries a `fontSize: 14` that reading alone would have suggested was a fixed kebab; it is in the popover, not the collapsed pill.)

**What that leaves, and it is not nothing.** The base should be set in a *relative* unit, not px. A px base pins the component against a reader who has raised their default font size — browser zoom scales px fine, but text-only resizing does not touch it, which is the property SC 1.4.4 is about. In `rem` arcviz gets exactly the control it wants **and** tracks the reader's preference.

**And then the floor form has to be expressed in em too, which is a substantive change.** DD-5 defines a floor as "a fixed form carrying the same content at any width" — but 76×44 px is not that. It is a box that fits exactly one type size, which is §11's whole finding. Expressed relatively it becomes a form that is genuinely the same at every base. The measurements show arcviz's own fixed paddings are what currently break the invariance: the pill converges cleanly on 7.5 em, while the chip's derived width drifts from 8.6 em down to 8.06 em across the same range purely because arcviz's padding, gaps and the port's border are fixed px.

**Not done, and deliberately.** Converting the layout to em is all-or-nothing — a relative base with a px chip would clip the moment a reader scales text, which is worse than either end — and it re-parameterises every packing formula in §3 against a base that the legibility probe has not set. Sequencing that before the probe means publishing a second set of formulas that the probe then moves again.
