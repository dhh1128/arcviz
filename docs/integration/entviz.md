...
**Consequence for arcviz, ruled by Daniel on 2026-09-24 (D-VTKV).** Posture is per value, as entviz intends — its own source says *"Provenance is per-VALUE, not per-viewport"* (`@entviz/core` `trust.ts:12-15`). For an **AID**, the value is in the corpus exactly when the host's alias lookup knows it, so membership can differ from one AID to the next. An aliased AID gets a corpus assumption, and an AID the lookup does not know gets none, which is wild: it shows `cesr digest` and no mnemonic. A **SAID** is never wild: *"SAIDs don't have a MITM risk and should never be 'wild'."* entviz has no way to express that by kind of value, so arcviz passes a corpus assumption on every SAID pill. An earlier version of this note treated posture as one decision for a whole presentation, and the sample briefly had a single "host treats AIDs as a corpus" checkbox. Both were wrong.
