...
**Defence.** P13: joins across separately-loaded presentations only as explicit, distinctly-styled, logged acts (Rule 14); no corpus-level negatives (Rule 13); "unlinkable"/"anonymous" never rendered as absolutes; the join log and retained material named as themselves a correlation store, with lifetime a host input and no-persistence the default posture (Rule 14's note; §f question 11). The papers corpus supplies the reasoning: making correlation cheap *is* the harm ([hardman-wbca-expense]), negative facts are its sharpest form ([hardman-wbca-negative]), and linking facets is "never a default, never an inference a stranger is meant to draw" ([hardman-if-owner]).
...
**Residual.** The verifier who wants the join gets it. The design succeeds if the join is *visible as an act* — logged, styled, never ambient — so that what the software condones is never silent. The retention bound is not yet designed (§f question 11 is open), and until it is, this threat's viewer-side-accumulation branch is acknowledged rather than mitigated.
...
- **Retention bounds.** §f question 11 (join-log and export lifetime, per-credential export permission, no-persistence default) is proposed but undecided; A8's accumulation branch stays open until it lands in the Phase 3/4 host-interface design.
