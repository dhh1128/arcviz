...
- **Scaffolding: full apparatus always.** Daniel Hardman's ruling. Every render carries the same structural honesty machinery regardless of audience — one code path, consistent, never under-warns, at the cost of legibility on the common case. **Open collision, flagged not resolved:** [affordances.md](affordances.md) AF17 holds that a holder-facing view shows *no presenter indicator at all, positive or negative*, "because the question is foreclosed, not unanswered", derived from `intent-boundaries.md:84`'s corollary against manufacturing boundaries that do not exist. Two readings are live — the ruling governs epistemic depth (H/C/V/T/X) and leaves the presenter exception standing, or it overrides AF17 and presenter provenance takes a positive mark even when the answer is "you", on the same logic that makes "unassessed" a grade rather than a blank. Proceeding on the first reading; the stress board is verifier-facing so nothing is blocked either way.
- **Static export carries a full proof-carrying payload.** Daniel's ruling, taken with the trade-off stated: a recipient can verify offline, and the holder's control over onward disclosure ends when the file leaves. This resolves one horn of the export dilemma recorded in [threat-model.md](threat-model.md); the charrette had settled only the export's *register* (worksheet, not certificate), not its contents. **Consequence to carry into the affordance inventory:** exporting becomes a *disclosure* act rather than a rendering one, so it takes an intent-boundary confirm rather than a silent affordance, and the correlation principles follow the artifact into a recipient's hands rather than stopping at the viewport.

## Decisions taken 2026-09-11

- **Scope: arcviz renders one loaded presentation at a time, and correlation highlighting is within that presentation.** Daniel Hardman's ruling, answering a question the corpus had three times declined to answer on his behalf. Cross-presentation joining by identifier equality is not a feature arcviz offers — it was never wanted. The consequences run wider than removing an affordance: the correlation rule becomes unconditional (no edge is drawn between presentations, explicit or otherwise), `if.md:63`'s prohibition on software linking facets is satisfied **by construction rather than by discipline**, the join log and the joinable retained store that a review finding worried about never come into being, and the correlating-verifier threat moves from *mitigated* to *out of scope* — what a determined operator does by hand outside the component was never a threat arcviz could carry. Recorded in [affordances.md](affordances.md) AF14 (retired), [disclosure-matrix.md](disclosure-matrix.md) Rule 14, [principles.md](principles.md) P13 and tension T1, and [threat-model.md](threat-model.md) A8.
