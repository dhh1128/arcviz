...
- **Substitution must be visible.** Every SAID and AID renders as an entviz pill, per the perceptual argument in `~/code/me/papers/amp-diff.md`, so a substituted identifier disturbs a picture rather than hiding in a string a human will not read.
