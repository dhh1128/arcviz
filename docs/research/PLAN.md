...
- **Unknown operators render as unevaluated, never as invalid.** When arcviz meets an edge operator it does not recognize — `E1E` is the live case, load-bearing in both guardianship worked examples but absent from the published spec — the honest render is "unknown operator, constraint unevaluated," which is the `unperformable` outcome on axis V. keripy's current coerce-and-reject behavior is the naive alternative those same worked examples criticize.
