...
- **The governing operator profile is a host-supplied input, per credential.** The premise that there is one operator vocabulary to check against is false. `trustoverip/kswg-acdc-specification` has two live branches with **different normative unary operator tables** — `main` (`f0bd097de`) lacks `E1E`, `v1.1` (`910c74afc`) adopted it in PR #197 and amended the injection clause to include it. keripy's `UnaryOps` matches neither exactly, its `DI2I` implementation is still an open PR (#1564), and the dossier spec mints four more operators (`MxN`, `RMxN`, `MxQ`, `RMxQ`) into the same `o` field. The effective operator set is therefore a property of the deployment, and nothing in an ACDC names it — the `v` field gives the serialization version, not the operator vocabulary in force. Resolve it per credential, like the freshness policy, and make "governing profile unsupplied" a visible state that every operator-dependent claim inherits.
